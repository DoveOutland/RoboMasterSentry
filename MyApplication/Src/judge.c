...
/*
** CRC验证函数
** 输入: 要验证的数据，流长度=数据+校验和
** 输出: True or False (CRC验证结果)
*/
...
void Judge_Data_Receive(uint8_t* ReadFromUsart)
{
    ...
    if(ReadFromUsart[0] == 0xa5)                   //判断帧头是否为0xa5
    {
        ...
        {
            judge_length = (ReadFromUsart[2] << 8 | ReadFromUsart[1]) + 5 + 2 + 2;	// ReadFromUsart[1]统计一帧数据长度,用于CR16校验 两个字节存储吗???(ReadFromUsart[2]<<8|ReadFromUsart[1])
            if(Verify_CRC16_Check_Sum(ReadFromUsart, judge_length) == 1) //帧尾CRC16校验
            {
                ...
                case 0x0201:
                    Judge_ID = 0x0201;
                    memcpy(&Robot_State, ReadFromUsart, 25);
                    break;

                case 0x0202:
                    Judge_ID = 0x0202;
                    memcpy(&Power_Heat, ReadFromUsart, 23);
                    break;
